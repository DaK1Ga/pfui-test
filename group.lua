pfUI:RegisterModule("group", function ()
  ...
  for i=1, 4 do
    pfUI.uf.group[i] = CreateFrame("Button","pfGroup" .. i,UIParent)
		...
		if pfUI_config.unitframes.group.group_layout == "raid" then
			pfUI.uf.group[i]:SetWidth(pfUI_config.unitframes.group.width)
			pfUI.uf.group[i]:SetHeight(40 + 2*default_border + spacing)
			pfUI.uf.group[i]:SetPoint("BOTTOMLEFT", 5 + ((i-1) * (pfUI_config.unitframes.group.width + default_border + 4)), pfUI_config.chat.left.height + 10 + default_border)
		else
    ...
    pfUI.uf.group[i].hp:SetWidth(pfUI_config.unitframes.group.width)
    ...
    pfUI.uf.group[i].power:SetWidth(pfUI_config.unitframes.group.width)
    ...
    pfUI.uf.group[i].caption:SetFont(pfUI.font_square, pfUI_config.global.font_size - 2, "OUTLINE")
